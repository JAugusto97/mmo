"""
Credits to the authors of the paper: Synthetic Oversampling of Multi-Label Data
based on Local Label Distribution (https://arxiv.org/pdf/1905.00609)

This code is a slight modification from the implementation
by diliadis (https://github.com/diliadis/mlsol/blob/master/MLSOL.py)
...
class MLSOL:
    ...
    def generate_instance(
        self, x_seed, y_seed, T_seed, x_reference, y_reference, T_reference
    ):
        x_synthetic = np.zeros(len(x_seed))
        y_synthetic = np.zeros(len(y_seed))
        for i in range(len(x_seed)):
            x_synthetic[i] = x_seed[i] + (
                random.random() * (x_reference[i] - x_seed[i])
            )
    ...
    def fit_resample(self, X, y):
        ...
        nbrs = NearestNeighbors(
            n_neighbors=self.k + 1, algorithm="brute", metric="euclidean"
        ).fit(X)
        ...
        for i in range(gen_num):
            ...
            X_synthesized[gen_num - 1], y_synthesized[gen_num - 1] = (
